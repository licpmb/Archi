...
ArchiPam 首先是面向 Agent 的 Skill。普通用户只需向支持 Skill 的 Agent 描述想要的图；不必学习 Schema，也不必自己运行 `validate`、`inspect` 或 `deliver`。

下面的手工流程面向集成、贡献和排错。命令假设你位于仓库的 `archipam/` 目录，或已经安装好的 ArchiPam Skill 根目录。

## 1. 检查安装

ArchiPam 要求 Node.js 18 或更高版本。开始编图前先运行 doctor：
...
```bash
node bin/archipam.mjs guide "展示带 Redis 缓存未命中的 API 请求" --json --lang zh
```
...
每个源文件都需要 `schema_version`、`diagram_type`、`meta.title`，以及对应 Renderer 要求的结构数组。精确字段和允许值请阅读 [Schema 说明](../archipam/schemas/README.md)。
...
ArchiPam 会校验 Git 远端、commit、blob 和请求的代码行。无法验证仓库或版本时，不要添加源码证据。
...
这份回执只证明有限范围内的运行时表现，并不批准视觉质量。仍需单独检查 HTML 或生成的截图。记录补充性的手工浏览器工作时必须遵循[交付契约](../archipam/references/delivery-contract.md)；不受约束的目检只影响视觉复核结论。

自动化浏览器证据的规范覆盖、成品绑定、视觉复核状态和交付回执字段请参阅交付契约。编图不变量和有上限的修复循环请参阅 [Skill 契约](../archipam/SKILL.md)。
