# ArchiPam first-class Cursor onboarding research

> Snapshot: 2026-07-23. Sources are limited to Cursor's official documentation and changelog, the official `vercel-labs/skills` repository and CLI, ArchiPam's own issue/docs/tests, and first-party peer repositories.

## Decision

**Recommendation:** add Cursor as a first-class installation surface for the existing `archipam` Skill. Do not fork `SKILL.md`, add a Cursor-only runtime, or call the work complete until one real Cursor Editor/CLI session discovers the Skill and produces a verified artifact.
...
npx -y skills add licpmb/Archi --skill archipam --agent cursor --copy --yes
```

For a reproducible test fixture, pin the version that was verified in this snapshot:

```bash
npx -y skills@1.5.20 add licpmb/Archi --skill archipam --agent cursor --copy --yes
```
...
| Scope | Valid Skill roots | ArchiPam entry |
|---|---|---|
| Project | `.agents/skills/`, `.cursor/skills/` | `<root>/archipam/SKILL.md` |
| User/global | `~/.agents/skills/`, `~/.cursor/skills/` | `<root>/archipam/SKILL.md` |
...
### ArchiPam's current evidence

- [Issue #46](https://github.com/licpmb/Archi/issues/46) is open and asks to collaborate on Cursor usage. It had no comments at this snapshot.
- The current ArchiPam README names Claude, Codex CLI, and opencode, and its installation matrix does not include Cursor.[Current positioning](https://github.com/licpmb/Archi/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/README.md#L11-L19) · [Current installation matrix](https://github.com/licpmb/Archi/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/README.md#L223-L231)
- ArchiPam requires Node.js 18 or newer, but its distributed renderer and validators do not require `npm install`.[Runtime contract](https://github.com/licpmb/Archi/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/archipam/package.json#L8-L13) · [Skill setup contract](https://github.com/licpmb/Archi/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/archipam/SKILL.md#L27-L33)
- Existing tests already simulate a copied installation with `node_modules` and repository-only tests removed, then exercise all five modes through `deliver` and run live preview.[Installed-copy fixture](https://github.com/licpmb/Archi/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/archipam/test/cli.test.mjs#L56-L69) · [Five-mode installed delivery](https://github.com/licpmb/Archi/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/archipam/test/cli.test.mjs#L357-L381) · [Installed preview](https://github.com/licpmb/Archi/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/archipam/test/cli.test.mjs#L383-L425)
...
```bash
npx -y skills@1.5.20 add licpmb/Archi \
  --skill archipam --agent cursor --copy --yes
```

Observed evidence:

1. `npx -y skills@1.5.20 list --agent cursor --json` reported one project-scoped `Cursor` installation at `.agents/skills/archipam`.
2. `node .agents/skills/archipam/bin/archipam.mjs doctor` exited 0.
...
|---|---|---|---|
| Global | Current CLI resolves Cursor to `~/.agents/skills/archipam`; Cursor officially scans that root. | A shell-capable Skill becomes available in every project; update timing is user-wide. | Default for an individual who wants ArchiPam everywhere. Say exactly what scope it grants. |
| Project | Installs `.agents/skills/archipam` plus `skills-lock.json` in the current repository. | The copied Skill is a substantial repository diff if committed, and can become stale relative to upstream. | Offer as the team/reproducible option, not as a silent default. |
...
- “Install ArchiPam for Cursor with `--agent cursor`; use `.agents/skills/archipam` for a project or a Cursor-supported global Skill root for user scope.”
- “The installed ArchiPam package passes `doctor`, validation, and checked HTML delivery without `npm install`, provided Node.js 18+ is available.”
- “ArchiPam uses the same portable `SKILL.md` and renderer contract across Cursor, Claude, Codex, and opencode.”
...
- “The global CLI command installs to `~/.cursor/skills/archipam`.” The current implementation resolves Cursor as universal and may use `~/.agents/skills/archipam`.
- “Cursor's Remote Rule UI imports ArchiPam correctly from the bare repository URL.” Cursor documents the UI, but not nested Skill selection; ArchiPam's actual Skill lives under `archipam/`.
...
3. Reuse the same `archipam/SKILL.md`; add no Cursor fork or Cursor-specific diagram behavior.
