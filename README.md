...
<p align="center">
  <a href="https://trendshift.io/repositories/31352?utm_source=repository-badge&amp;utm_medium=badge&amp;utm_campaign=badge-repository-31352" target="_blank" rel="noopener noreferrer"><img src="https://trendshift.io/api/badge/repositories/31352" alt="ArchiPam on Trendshift" width="250" height="55"/></a>
</p>

![ArchiPam product preview](docs/assets/archipam-readme-hero.png)

# ArchiPam

**Turn a codebase or system description into a polished, interactive system map — directly in chat.**

ArchiPam is a Node.js rendering and validation system for Cursor, Claude Code, Codex CLI, and OpenCode. Agents produce typed JSON IR; ArchiPam deterministically compiles it into HTML/SVG.
...
**[Project page](https://licpmb.github.io/Archi/)** · **[Scenario guide](https://licpmb.github.io/Archi/guide.html)** · **[Proof Lab](https://licpmb.github.io/Archi/gallery.html)**

```bash
npx skills add licpmb/Archi -g
```

Using Cursor? Open the [agent-aware quick start](https://licpmb.github.io/Archi/start.html?agent=cursor&type=architecture) for exact global and project commands.
...
<p align="center">
  <a href="https://licpmb.github.io/Archi/gallery.html"><img src="docs/assets/archipam-live-proof.gif" alt="Three verified ArchiPam artifacts moving through Signal Flow, Blueprint, and Classic presets" width="960"/></a>
  <br/>
  <sub><strong>Three real generated artifacts.</strong> Signal Flow · Blueprint · Classic · <a href="https://licpmb.github.io/Archi/gallery.html">open the interactive Proof Lab ↗</a></sub>
</p>

| Guided story | Route probe | Semantic lens |
|---|---|---|
| [![Agent workflow playing one authored chapter](docs/assets/archipam-demo-story.png)](https://licpmb.github.io/Archi/gallery/artifacts/agent-tool-call.workflow.html?theme=dark&present=1&play=1#view=happy-path) | [![Cache-miss sequence showing the Web App to Postgres route](docs/assets/archipam-demo-route.png)](https://licpmb.github.io/Archi/gallery/artifacts/cache-miss.sequence.html?theme=dark&present=1#route=web~db) | [![Production architecture comparing backend and database roles](docs/assets/archipam-demo-lens.png)](https://licpmb.github.io/Archi/gallery/artifacts/production-deployment.architecture.html?theme=dark&present=1#lens=backend~database) |
| Play one finite named chapter. | Inspect the shortest authored directed path. | Compare real traffic between semantic roles. |

The [Proof Lab](https://licpmb.github.io/Archi/gallery.html) contains all 11 checked-in scenarios, their JSON sources, named views, and validation receipts.

### A real repository, mapped from source

[![MCO runtime architecture generated from the public mco-org/mco repository](docs/assets/mco-runtime-share-card.png)](https://licpmb.github.io/Archi/cases/mco-runtime.architecture.html?theme=dark&present=1#view=dispatch-path)

ArchiPam traced [`mco-org/mco`](https://github.com/mco-org/mco) at `9f1a1cf` and produced this checked map. **[Open it ↗](https://licpmb.github.io/Archi/cases/mco-runtime.architecture.html?theme=dark&present=1#view=dispatch-path)** · [trace reach ↗](https://licpmb.github.io/Archi/cases/mco-runtime.architecture.html?theme=dark#focus=router&reach=downstream) · [typed source](docs/cases/mco-runtime.architecture.json)
...
| Dark | Light |
|---|---|
| ![Dark theme](docs/assets/archipam-dark.png) | ![Light theme](docs/assets/archipam-light.png) |

The Export menu copies PNG to the clipboard and downloads static or motion formats:

![Export menu](docs/assets/archipam-menu.png)
...
![Route Share Card showing the exact Users to API Server path with the full architecture retained as context](docs/assets/archipam-route-share-card.png)
...
```bash
npx -y skills add licpmb/Archi --skill archipam --agent cursor --global --copy --yes
```

To try without installing:

```bash
npx skills use licpmb/Archi@archipam --agent codex
```

[DSH community opt-in](integrations/deepseek-harness/README.md): `dsh plugin --profile web add @licpmb/archipam-dsh@0.1.0`

The [agent switcher](https://licpmb.github.io/Archi/start.html?agent=cursor&type=architecture) covers `cursor`, `codex`, `claude-code`, and `opencode`. For Raven's manual ZIP install, extract [`archipam.zip`](archipam.zip) into `~/.raven/workspace/skills`; it yields `~/.raven/workspace/skills/archipam`. Raven is not a switcher target.

ArchiPam may GET the fixed stable manifest solely to show an optional reminder; it never downloads or installs updates. Successful checks wait about 72 hours (±20%); active use retries failures after 6, then 24 hours. The server sees normal HTTP metadata (IP and time), but receives no version, Agent, project data, prompts, account/device ID, or ETag. You decide whether and when to update. Set `ARCHIPAM_UPDATE_CHECK_DISABLED=1` to disable networking and reminder-state writes.

### 2. Start from a description — no repository required

```text
Use ArchiPam to draw: Browser -> API -> Redis cache -> PostgreSQL fallback.
```

For source evidence, open a repository and ask:

```text
Analyze this repository, then use archipam to create a high-level runtime architecture diagram.
...
Continue with focused requests such as `add Redis`, `move auth to the left`, or `highlight the rollback path`. ArchiPam keeps the typed source available for targeted iteration.
...
`node archipam/bin/archipam.mjs compare architecture base.json head.json architecture-delta.html --json`
...
Not sure which one fits? Use the [interactive scenario guide](https://licpmb.github.io/Archi/guide.html), or ask the zero-dependency CLI:

```bash
node archipam/bin/archipam.mjs guide "Show an API request with Redis cache miss"
node archipam/bin/archipam.mjs guide "Map Kafka topics, consumer groups, replay, and DLQ" --json
```

Workflow keeps the happy path clear across lanes:

![Workflow example](docs/assets/archipam-workflow.png)

Sequence explains one interaction over time:

![Sequence example](docs/assets/archipam-sequence.png)

Data Flow makes movement and sensitivity boundaries explicit:

![Data Flow example](docs/assets/archipam-dataflow.png)

Lifecycle separates progress, waits, retries, and terminal outcomes:

![Lifecycle example](docs/assets/archipam-lifecycle.png)

Architecture examples: [`web-app`](examples/web-app.html) · [`ArchiPam pipeline`](examples/archipam-repo.html) · [`grid placement`](examples/archipam-repo-grid.html) · [`desktop agent`](examples/maka-architecture.html)
...
ArchiPam is not a general-purpose drawing editor or a Mermaid theme. It turns technical intent into a communication artifact.
...
```bash
cd archipam
node bin/archipam.mjs doctor
node bin/archipam.mjs demo /tmp/archipam-demo
node bin/archipam.mjs guide "Show CI/CD checks, approval, deploy, and rollback"
node bin/archipam.mjs validate workflow examples/agent-tool-call.workflow.json --quality showcase --json
node bin/archipam.mjs preview workflow examples/agent-tool-call.workflow.json /tmp/workflow.html --quality showcase
node bin/archipam.mjs deliver workflow examples/agent-tool-call.workflow.json /tmp/workflow.html --quality showcase --open --json
```
...
The complete generation and viewer contract lives in [`archipam/SKILL.md`](archipam/SKILL.md).

## Installation options

| Surface | Install location or method | Capability |
|---|---|---|
| **Raven** | Manual ZIP into `~/.raven/workspace/skills` → `~/.raven/workspace/skills/archipam` | Full renderer + validation workflow |
...
| **Claude.ai** | Upload `archipam.zip` under Settings → Capabilities → Skills | Depends on Node.js access in the sandbox |
| **Project Knowledge** | Upload `archipam.zip` to the project | Prompt-driven architecture fallback |
**DeepSeek Harness:** Community integration, not an official DeepSeek product; developer-preview `@deepseek-ai/dsh@0.1.0-rc.6`, Node `^22.19.0 || >=24.0.0`. Install: `dsh plugin --profile web add @licpmb/archipam-dsh@0.1.0`; invoke: `Use the archipam skill to map this repository's runtime architecture.`; remove: `dsh plugin --profile web remove @licpmb/archipam-dsh`. No telemetry. Shell files need exact workspace paths, not Web Produced Files. [Details](integrations/deepseek-harness/README.md).

## Reference and scope

- [Schema reference](archipam/schemas/README.md) · [Skill](archipam/SKILL.md) · [Examples](archipam/examples/) · [Agent cookbook](docs/authoring-cookbook.md)
- [Changelog](CHANGELOG.md)
- [Roadmap](ROADMAP.md)
- [Generated Proof Lab](https://licpmb.github.io/Archi/gallery.html)
...
Issues, pull requests, and real-world diagrams are welcome. Start with the [contribution guide](CONTRIBUTING.md), use the reproducible bug form for failures, or submit a validated diagram through the [community showcase form](https://github.com/licpmb/Archi/issues/new?template=showcase.yml).&nbsp;·&nbsp;[LINUX&nbsp;DO](https://linux.do)

## Star History

<p align="center"><picture><source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/licpmb/Archi/star-history/assets/star-history-dark.svg" /><img alt="Star History" src="https://raw.githubusercontent.com/licpmb/Archi/star-history/assets/star-history-light.svg" /></picture></p>
