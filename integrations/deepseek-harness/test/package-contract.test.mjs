...
test('publishable manifest is @licpmb/archipam-dsh@0.1.0 with a DSH bundle patch and no install surface', () => {
  const pkg = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  assert.equal(pkg.name, '@licpmb/archipam-dsh');
  ...
  assert.equal(pkg.repository?.url, 'git+https://github.com/licpmb/Archi.git');
...
test('bundle patch inserts one uniquely named filesystem Skill provider resolved from the installed package', () => {
  ...
  assert.match(patch, /id:\s*archipam-skill-filesystem/);
  assert.match(patch, /name:\s*'@deepseek-ai\/dsh-skill-filesystem'/);
  assert.match(patch, /providerName:\s*archipam-plugin/);
  ...
  assert.deepEqual(insertedIds, ['archipam-skill-filesystem']);
});
