...
test('host-loaded adapter code does not open a second execution, network, credential, or telemetry surface', () => {
  ...
  assert.doesNotMatch(source, /archipam_render|archipam_deliver|tools\.register/);
});

test('package resolution failures fail loud instead of returning a guessed Skill root', async () => {
  const { resolveArchiPamSkillRoot } = await import('../lib/index.js');
  assert.throws(() => resolveArchiPamSkillRoot('file:///tmp/archipam-dsh-missing-profile/'));
  assert.throws(() => resolveArchiPamSkillRoot(''));
});
