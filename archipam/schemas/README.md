...
explicit authored viewBox without overlap, ArchiPam omits the complete legend
...
by `archipam brands --json`, or a digest-pinned `{ "url", "sha256" }` object
returned by `archipam brands capture <url> --json`. Known IDs and known-brand
...
can be produced explicitly with `archipam migrate workflow ... --to-schema 2`.
...
strictly ArchiPam judges composition. `engineering_profile` is an optional
