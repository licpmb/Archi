...
function schemaDiagnostics(workflow) {
  try {
    ...
    if (Array.isArray(error?.archipamDiagnostics)) {
      return error.archipamDiagnostics.map((entry) => ({ ...entry }));
    }
    throw error;
  }
}
  ...
  try {
    ...
    return result({
      ...
      migrationDiagnostics: [diagnostic({
        ...
        supportedFixes: ['report the workflow and compiler receipts to the ArchiPam maintainers'],
      })],
      oldRequiredViewBox,
      newRequiredViewBox: requiredViewBoxFrom(planned),
    });
