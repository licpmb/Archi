...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-presentation-'));
...
test('all typed renderers ship the same presentation stage contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /ArchiPam\.presentation = \(function \(\)/, mode);
...
test('presentation keyboard behavior exits in layers and remains accessible', () => {
  ...
  assert.match(html, /ArchiPam\.presentation\.toggle\(\)/);
  assert.match(html, /e\.key === 'Escape' && ArchiPam\.focus\.active\(\)/);
  assert.match(html, /e\.key === 'Escape' && ArchiPam\.presentation\.active\(\)/);
