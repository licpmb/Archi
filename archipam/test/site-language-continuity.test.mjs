...
const integrationEnabled = process.env.ARCHIPAM_SITE_INTEGRATION === '1';
const chromePath = integrationEnabled && process.env.ARCHIPAM_CHROME ? findChrome() : null;
...
function loadRuntime({
  ...
  return { language: window.ArchiPamSiteLanguage, values, url: () => new URL(currentUrl) };
}
...
test('site language runtime normalizes one entry parameter into one durable preference', () => {
  const canonical = loadRuntime({ values: new Map([['archipam-lang', 'zh']]) });
  assert.equal(canonical.language.read(), 'zh');

  for (const legacyKey of ['archipam-gallery-language', 'archipam-guide-language']) {
    ...
    assert.equal(legacy.values.get('archipam-lang'), 'zh', `${legacyKey} must migrate to the canonical key`);
  }

  const canonicalWins = loadRuntime({
    values: new Map([
      ['archipam-lang', 'en'],
      ['archipam-gallery-language', 'zh'],
      ['archipam-guide-language', 'zh'],
    ]),
  });
  ...
  const secondLegacyFallback = loadRuntime({
    values: new Map([
      ['archipam-gallery-language', 'fr'],
      ['archipam-guide-language', 'zh'],
    ]),
  });
  assert.equal(secondLegacyFallback.language.read(), 'zh');
  assert.equal(secondLegacyFallback.values.get('archipam-lang'), 'zh');

  const conflictingLegacy = loadRuntime({
    values: new Map([
      ['archipam-gallery-language', 'zh'],
      ['archipam-guide-language', 'en'],
    ]),
  });
  assert.equal(conflictingLegacy.language.read(), 'zh');
  assert.equal(conflictingLegacy.values.get('archipam-lang'), 'zh');
  conflictingLegacy.values.set('archipam-gallery-language', 'en');
  ...
  const explicit = loadRuntime({
    ...
    values: new Map([['archipam-lang', 'zh']]),
  });
  assert.equal(explicit.language.read(), 'en');
  assert.equal(explicit.values.get('archipam-lang'), 'en');
  ...
  const historyBlocked = loadRuntime({
    ...
    values: new Map([['archipam-lang', 'en']]),
    historyError: true,
  });
  assert.equal(historyBlocked.language.read(), 'zh');
  assert.equal(historyBlocked.values.get('archipam-lang'), 'zh');
  ...
  assert.match(source, /archipam-gallery-language/);
  assert.match(source, /archipam-guide-language/);
...
test('custom site builders emit every shared site asset and preserve entry, navigation, selection, and refresh state', {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-site-language-'));
  try {
    const builds = [
    ...
    assert.equal(values.get('archipam-lang'), 'zh');
    ...
    assert.equal(values.get('archipam-lang'), 'en');
...
test('all site pages consume one language runtime and one navigation contract', () => {
  ...
  for (const relative of pages) {
    ...
    assert.match(html, /ArchiPamSiteLanguage\.read\(/, `${relative}: shared language read missing`);
    assert.match(html, /ArchiPamSiteLanguage\.write\(/, `${relative}: shared language write missing`);
    ...
    assert.doesNotMatch(
      html,
      /localStorage\.setItem\(['"]archipam-(?:lang|gallery-language|guide-language)['"]/,
...
test('real Chrome preserves language through entry, navigation, selection, refresh, and consistent navigation chrome', {
  skip: chromePath ? false : 'Set ARCHIPAM_CHROME to run the real site regression.',
  timeout: 60000,
}, async () => {
  ...
  try {
    ...
    await evaluate(browser, sessionId, "localStorage.setItem('archipam-guide-language', 'zh')");
    await navigate(browser, sessionId, `${baseUrl}/index.html`);
    assert.deepEqual(await evaluate(browser, sessionId, `({
      language: document.documentElement.lang,
      stored: localStorage.getItem('archipam-lang')
