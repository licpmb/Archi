...
function fixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-evidence-repo-'));
  ...
  git(root, 'init');
  git(root, 'config', 'user.name', 'ArchiPam Tests');
  git(root, 'config', 'user.email', 'archipam@example.test');
...
test('repository evidence is revision-verified, receipt-backed, searchable, and export-clean', () => {
  ...
  assert.match(html, /ArchiPam\.sourceEvidence = \(function \(\)/);
  ...
  assert.match(html, /ArchiPam\.sourceEvidence\.installBeacons\(\)/);
...
test('repository evidence is opt-in and never appears in ordinary artifacts', () => {
  const output = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-no-evidence-')), 'plain.html');
  const input = path.join(skillRoot, 'examples', 'web-app.architecture.json');
  const result = run(['render', 'architecture', input, output]);
  assert.equal(result.status, 0, result.stderr);
  const html = fs.readFileSync(output, 'utf8');
  assert.doesNotMatch(html, /id="archipam-source-evidence-data"/);
