...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-chapter-handoff-'));
...
test('handoff holds one truthful anchor then settles through one finite camera transaction', () => {
  ...
  assert.match(html, /ArchiPam\.motionGovernor\.claim\('handoff'/);
  assert.match(html, /ArchiPam\.motionGovernor\.release\(handoff\.ownerToken\)/);
...
test('latest intent, manual takeover, Still, reduced motion, and hidden pages cleanly settle', () => {
  ...
  assert.match(html, /ArchiPam\.guidedViews\.cancelHandoff\(reason \|\| 'manual'\)/);
