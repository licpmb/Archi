...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-webm-artifact-'));
const externalReachSource = process.env.ARCHIPAM_REACH_CARD_SOURCE
  ? path.resolve(process.env.ARCHIPAM_REACH_CARD_SOURCE)
  : '';
const externalReachOutput = process.env.ARCHIPAM_REACH_CARD_OUTPUT
  ? path.resolve(process.env.ARCHIPAM_REACH_CARD_OUTPUT)
  : '';
assert.equal(Boolean(externalReachSource), Boolean(externalReachOutput), 'ARCHIPAM_REACH_CARD_SOURCE and ARCHIPAM_REACH_CARD_OUTPUT must be set together');
...
function executable(candidates) {
  for (const candidate of candidates) {
    ...
    try {
      return execFileSync('sh', ['-c', `command -v "$1"`, 'archipam-which', candidate], { encoding: 'utf8' }).trim();
    } catch (_) {
...
const ffmpeg = executable([process.env.ARCHIPAM_FFMPEG, 'ffmpeg']);

assert.ok(chrome, 'Chrome/Chromium is required for the WebM artifact smoke test (or set ARCHIPAM_CHROME)');
assert.ok(ffmpeg, 'ffmpeg is required for the WebM artifact smoke test (or set ARCHIPAM_FFMPEG)');
...
try {
  ...
  async function verifyResolvedLegendContract(outputs) {
    async function inspectKinds(file, expectedKinds, theme) {
      ...
      await navigateReady(url, '!!(window.ArchiPam && ArchiPam.semanticLens)', `legend ${theme}`);
    ...
    await navigateReady(outputs.dataflow, '!!(window.ArchiPam && ArchiPam.semanticLens && ArchiPam.exportMenu)', 'Dataflow database legend runtime');
    const databaseRuntime = await evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      URL.createObjectURL = function (blob) {
        captured = blob.text();
        return 'blob:archipam-dataflow-legend-smoke';
      };
      HTMLAnchorElement.prototype.click = function () {};
      try {
        await ArchiPam.exportMenu.run('svg');
        var exportedText = await captured;
        var exported = new DOMParser().parseFromString(exportedText, 'image/svg+xml').documentElement;
        return {
          selected: ArchiPam.semanticLens.active(),
          lensOpen: ArchiPam.semanticLens.isOpen(),
    ...
    await navigateReady(outputs.custom, '!!(window.ArchiPam && ArchiPam.semanticLens && ArchiPam.exportMenu)', 'custom legend runtime');
    const runtime = await evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var selected = ArchiPam.semanticLens.active();
      var guidedActivated = ArchiPam.guidedViews.activate('main', { updateUrl: false });
      var guidedActive = ArchiPam.guidedViews.active();
      var visualMatrix = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!ArchiPam.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
      ...
      URL.createObjectURL = function (blob) {
        captured = blob.text();
        return 'blob:archipam-legend-smoke';
      };
      HTMLAnchorElement.prototype.click = function () {};
      try {
        await ArchiPam.exportMenu.run('svg');
    ...
    await navigateReady(embedUrl, '!!(window.ArchiPam && ArchiPam.semanticLens)', 'embedded legend');
    ...
    await navigateReady(outputs.hidden, '!!(window.ArchiPam && ArchiPam.semanticLens)', 'hidden legend');
  ...
  async function verifySemanticPassportDismissal(file) {
    await navigateReady(file, '!!(window.ArchiPam && ArchiPam.focus && document.querySelector("#btn-focus-clear"))', 'Semantic Passport dismissal');
    const result = await evaluate(cdp, sessionId, `(() => {
      ...
      function state() {
        return { hidden: chip.hidden, active: ArchiPam.focus.active() };
      }

      ArchiPam.focus.set('clients', { toggle: false, updateUrl: false });
  ...
  async function verifyArchitectureDeltaNavigator(file) {
    ...
    const exportProof = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var explorers = frames.map(function (frame) {
        ...
        return Boolean(child && child.ArchiPam && child.ArchiPam.focus && child.ArchiPam.routeProbe && child.document.querySelector('#btn-node-finder') && child.document.querySelector('#guided-view-play'));
      });
      var svgA = ArchiPam.deltaExport.canonicalSvg();
      document.querySelector('#theme').click();
      document.querySelector('#preset').click();
      document.querySelector('.change-row').click();
      var svgB = ArchiPam.deltaExport.canonicalSvg();
      var parsed = new DOMParser().parseFromString(svgB, 'image/svg+xml');
      var exportStyle = parsed.querySelector('style')?.textContent || '';
      var blob = await ArchiPam.deltaExport.shareCard();
  ...
  async function captureShareCard(file, label) {
    await navigateReady(file, '!!(window.ArchiPam && ArchiPam.exportMenu && ArchiPam.exportMenu.shareCard)', label);
    const sharePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        var blob = await ArchiPam.exportMenu.shareCard();
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
  ...
  async function captureCopiedShareCard(file, label) {
    await navigateReady(file, '!!(window.ArchiPam && ArchiPam.exportMenu && ArchiPam.exportMenu.copyShareCard)', label);
        ...
        Object.defineProperty(navigator, 'clipboard', {
          ...
          value: {
            write: async function (items) {
              window.__archipamCopiedShareCard = await Promise.resolve(items[0].items['image/png']);
            }
          }
        });
        window.alert = function (message) { window.__archipamCopyAlert = message; };
        await ArchiPam.exportMenu.copyShareCard();
        var blob = window.__archipamCopiedShareCard;
        if (!blob) throw new Error(window.__archipamCopyAlert || 'clipboard received no blob');
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
        for (var offset = 0; offset < bytes.length; offset += 32768) {
          binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
  ...
  async function captureRouteShareCard(file, label, sourceId, targetId, options = {}) {
    await navigateReady(file, '!!(window.ArchiPam && ArchiPam.routeProbe && ArchiPam.exportMenu && ArchiPam.exportMenu.downloadRouteShareCard)', label);
    const routePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__archipamRouteAlert = message; };
        ArchiPam.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
        if (!ArchiPam.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
          throw new Error('route did not resolve');
        }
        var snapshot = ArchiPam.routeProbe.exportSnapshot();
        if (!snapshot) throw new Error('resolved route exposed no export snapshot');
        ArchiPam.exportMenu.syncRouteShare();
        ...
        var duplicateGeometryRejected = ArchiPam.routeProbe.exportSnapshot() === null;
        var duplicateExportError = '';
        try { await ArchiPam.exportMenu.shareCard({ variant: 'route' }); }
        ...
        var emptyGeometryRejected = ArchiPam.routeProbe.exportSnapshot() === null;
        ...
        try {
          blob = await ArchiPam.exportMenu.downloadRouteShareCard();
          routeReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
          ...
          if (${JSON.stringify(options.journeyInvariance === true)}) {
            async function captureJourneySource(action) {
              ...
              await ArchiPam.exportMenu.shareCard({ variant: 'route' });
              routeFingerprints.push(fingerprint(await captured[captureIndex]));
            }
            await captureJourneySource(function () { ArchiPam.routeProbe.selectJourneyIndex(0); });
            await captureJourneySource(function () { ArchiPam.routeProbe.selectJourneyIndex(Math.floor(snapshot.nodeIds.length / 2)); });
            await captureJourneySource(function () { ArchiPam.routeProbe.selectJourneyIndex(snapshot.nodeIds.length - 1); });
            ArchiPam.routeProbe.showOverview({ reveal: false });
            var started = ArchiPam.routeProbe.playJourney();
            ...
            ArchiPam.routeProbe.pauseJourney({ preserveElapsed: false });
            await captureJourneySource(function () {});
          ...
          await ArchiPam.exportMenu.run('png');
          var pngReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            canonical: document.documentElement.getAttribute('data-last-export-canonical')
          ...
          var asyncClearPromise = ArchiPam.exportMenu.shareCard({ variant: 'route' });
          ArchiPam.routeProbe.clear({ updateUrl: false, preserveView: true });
          ...
          await ArchiPam.exportMenu.downloadRouteShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
          ...
          return {
            ...
            staleSnapshot: ArchiPam.routeProbe.exportSnapshot(),
  ...
  async function verifyDynamicReducedMotionRoute(file, label, sourceId, targetId) {
    ...
    await navigateReady(file, '!!(window.ArchiPam && ArchiPam.routeProbe && ArchiPam.exportMenu && ArchiPam.motionGovernor)', label);
    ...
    const setup = await evaluate(cdp, sessionId, `(function () {
      ArchiPam.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!ArchiPam.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) return { resolved: false };
      ArchiPam.routeProbe.showOverview({ reveal: false });
      return {
        resolved: true,
        started: ArchiPam.routeProbe.playJourney(),
        playing: ArchiPam.routeProbe.isJourneyPlaying(),
  ...
  async function captureRouteVisualMatrix(file, label, sourceId, targetId) {
    await navigateReady(file, '!!(window.ArchiPam && ArchiPam.preset && ArchiPam.routeProbe && ArchiPam.exportMenu)', label);
    const matrix = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      ArchiPam.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!ArchiPam.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
        throw new Error('route did not resolve');
      }
      var identity = JSON.stringify(ArchiPam.routeProbe.exportSnapshot());
      var results = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!ArchiPam.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
        for (var theme of ['dark', 'light']) {
          document.documentElement.setAttribute('data-theme', theme);
          var blob = await ArchiPam.exportMenu.shareCard({ variant: 'route' });
          var bytes = new Uint8Array(await blob.arrayBuffer());
          var hash = 2166136261;
          ...
          results.push({
            ...
            identity: JSON.stringify(ArchiPam.routeProbe.exportSnapshot())
          });
        }
      }
  ...
  async function captureReachShareCard(file, label, originId, direction, options = {}) {
    await navigateReady(file, '!!(window.ArchiPam && ArchiPam.focus && ArchiPam.focus.reachabilitySnapshot && ArchiPam.exportMenu && ArchiPam.exportMenu.downloadReachShareCard)', label);
    const reachPayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__archipamReachAlert = String(message); };
        if (!ArchiPam.focus.set(${JSON.stringify(originId)}, { toggle: false, updateUrl: false })) {
          throw new Error('focus origin did not resolve');
        }
        if (!ArchiPam.focus.reach(${JSON.stringify(direction)}, { toggle: false, updateUrl: false, reveal: false })) {
          throw new Error('authored reach did not resolve');
        }
        var snapshot = ArchiPam.focus.reachabilitySnapshot();
        if (!snapshot) throw new Error('active authored reach exposed no export snapshot');
        ArchiPam.exportMenu.syncReachShare();
        ...
        svg.appendChild(duplicateCarrier);
        var duplicateGeometryRejected = ArchiPam.focus.reachabilitySnapshot() === null;
        var duplicateExportError = '';
        try { await ArchiPam.exportMenu.shareCard({ variant: 'reach' }); }
        catch (error) { duplicateExportError = String(error && error.message || error); }
        duplicateCarrier.remove();
        snapshot = ArchiPam.focus.reachabilitySnapshot();
        ...
        try {
          var blob = await ArchiPam.exportMenu.shareCard({ variant: 'reach' });
          var reachSvgText = captured[0] ? await captured[0] : '';
          var downloadedBlob = await ArchiPam.exportMenu.downloadReachShareCard();
          var reachReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            width: document.documentElement.getAttribute('data-last-export-width'),
          ...
          if (${JSON.stringify(options.matrix === true)}) {
            ...
            for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
              ...
              for (var theme of ['dark', 'light']) {
                document.documentElement.setAttribute('data-theme', theme);
                var matrixBlob = await ArchiPam.exportMenu.shareCard({ variant: 'reach' });
                ...
                matrix.push({
                  ...
                  identity: JSON.stringify(ArchiPam.focus.reachabilitySnapshot())
                });
              }
            }
          ...
          ArchiPam.focus.clearReach({ updateUrl: false });
          ArchiPam.exportMenu.syncReachShare();
          ...
          await ArchiPam.exportMenu.downloadReachShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            errorFormat: document.documentElement.getAttribute('data-last-export-error-format'),
          ...
          var canonicalIndex = captured.length;
          var canonicalBlob = await ArchiPam.exportMenu.shareCard();
          var canonicalSvgText = captured[canonicalIndex] ? await captured[canonicalIndex] : '';
          var canonicalSvg = parser.parseFromString(canonicalSvgText, 'image/svg+xml').documentElement;
          ...
          return {
            ...
            hiddenAfterClear: hiddenAfterClear,
            staleSnapshot: ArchiPam.focus.reachabilitySnapshot(),
            staleError: staleError,
            failedReceipt: failedReceipt,
            canonicalSize: canonicalBlob.size,
  ...
  if (externalReachSource) {
    ...
    await captureReachShareCard(
      externalReachSource,
      'external-reach',
      process.env.ARCHIPAM_REACH_CARD_ORIGIN || 'router',
      process.env.ARCHIPAM_REACH_CARD_DIRECTION || 'downstream',
  ...
  await navigateReady(output, '!!(window.ArchiPam && ArchiPam.motion && ArchiPam.motion.canRecord())', 'motion artifact');

  const payload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
    try {
      var blob = await ArchiPam.motion.recordWebm({ duration: 1400, fps: 12 });
      var bytes = new Uint8Array(await blob.arrayBuffer());
      var binary = '';
      for (var offset = 0; offset < bytes.length; offset += 32768) {
        binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
