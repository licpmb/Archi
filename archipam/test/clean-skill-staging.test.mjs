...
function repositoryFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-clean-stage-'));
  write(root, 'archipam/package.json', JSON.stringify({
    name: 'archipam-fixture',
    scripts: { test: 'node --test' },
    devDependencies: { ajv: '1.0.0' },
  }));
  write(root, 'archipam/package-lock.json', '{}\n');
  write(root, 'archipam/skill-release.json', '{}\n');
  write(root, 'archipam/scripts/check-update.mjs', 'export {};\n');
  write(root, 'archipam/scripts/update-contract.mjs', 'export {};\n');
  write(root, 'archipam/renderers/shared/generated-validators.mjs', 'export {};\n');
  write(root, 'archipam/test/repository-only.test.mjs', 'throw new Error();\n');
  git(root, ['init']);
  return root;
}
...
test('clean staging preserves index modes and strips repository-only package metadata', () => {
  ...
  try {
    write(root, 'archipam/bin/executable.mjs', '#!/usr/bin/env node\n', 0o755);
    write(root, 'archipam/runtime/test/required.dat', 'runtime fixture\n');
    git(root, ['add', 'archipam']);
...
test('clean staging rejects a symlink in a tracked file ancestor before copying bytes', (t) => {
  ...
  const destination = path.join(root, 'staged-skill');
  try {
    const runtime = path.join(root, 'archipam', 'runtime');
    write(root, 'archipam/runtime/payload.txt', 'tracked fixture\n');
    git(root, ['add', 'archipam']);
    ...
    assert.throws(
      () => stageCleanSkill({ repoRoot: root, destination }),
      /refusing to package path through symlink: archipam\/runtime/,
    );
...
test('clean staging rejects tracked symlinks before reading through them', (t) => {
  ...
  try {
    ...
    const linked = path.join(root, 'archipam', 'linked.txt');
    try {
    ...
    assert.throws(
      () => stageCleanSkill({ repoRoot: root, destination }),
      /refusing to package tracked symlink: archipam\/linked\.txt/,
    );
    assert.equal(fs.existsSync(destination), false);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
...
test('clean staging snapshots unstaged tracked bytes before a source ancestor can be swapped', (t) => {
  ...
  let swapped = false;
  try {
    const payload = write(root, 'archipam/runtime/payload.txt', 'indexed fixture\n');
    write(root, 'outside-runtime/payload.txt', 'external secret\n');
    git(root, ['add', 'archipam']);
...
test('clean staging rejects a source ancestor swapped during preflight traversal', (t) => {
  ...
  try {
    ...
    write(root, 'outside-runtime/payload.txt', 'external secret\n');
    git(root, ['add', 'archipam']);
    const canonicalRuntime = path.join(fs.realpathSync(root), 'archipam', 'runtime');
    ...
    assert.throws(
      () => stageCleanSkill({ repoRoot: root, destination }),
      /(?:tracked package path changed before it could be read: archipam\/|tracked package input is missing or unreadable: archipam\/runtime\/payload\.txt)/,
    );
...
test('clean staging reports the Git spawn error when Git cannot start', () => {
  ...
  try {
    ...
    assert.match(result.stderr, /unable to enumerate tracked ArchiPam files: .*ENOENT/);
    assert.doesNotMatch(result.stderr, /tracked ArchiPam paths must be valid UTF-8/);
    assert.equal(fs.existsSync(destination), false);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
