...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-animation-'));
...
test('signal-flow preset reaches the page, SVG, and motion export surface', () => {
  ...
  assert.match(html, /ArchiPam\.motion = \{ canRecord: canRecordMotion, recordWebm: recordWebm \}/);
  assert.match(html, /recorder\.requestData\(\)/);
  assert.match(html, /aria-label="Diagram view controls"/);
  assert.match(html, /ArchiPam\.focus = \(function \(\)/);
  assert.match(html, /ArchiPam\.view = \(function \(\)/);
...
test('editorial preset reaches every visual surface and all five typed renderers', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /data-preset-badge-editorial-plate="ARCHIPAM \/ PLATE 04"/, mode);
...
for (const [mode, example] of Object.entries(CASES)) {
  test(`${mode}: trace animation annotates svg, edges, and nodes`, () => {
    ...
    assert.match(svg, /aria-labelledby="archipam-diagram-title archipam-diagram-description"/);
    assert.match(svg, /<title id="archipam-diagram-title">[^<]+<\/title>/);
    assert.match(svg, /<desc id="archipam-diagram-description">[^<]+<\/desc>/);
