...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-gallery-'));
...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.equal(manifest.archipamVersion, JSON.parse(fs.readFileSync(path.join(skillRoot, 'package.json'))).version);
