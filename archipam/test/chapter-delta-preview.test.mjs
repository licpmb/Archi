...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-chapter-delta-preview-'));
...
test('pointer and keyboard inspect while touch and native activation still commit directly', () => {
  ...
  assert.doesNotMatch(previewRuntime, /ArchiPam\.view\.|updateUrl\(|ArchiPam\.focus\.|renderStoryTrail\(/);
});
...
test('latest intent, stronger owners, playback, and lifecycle cleanup remain bounded', () => {
  ...
  assert.match(html, /ArchiPam\.motionGovernor\.claim\('chapter-preview'/);
  assert.match(html, /ArchiPam\.motionGovernor\.release\(token\)/);
