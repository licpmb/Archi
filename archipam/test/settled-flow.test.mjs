...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-settled-flow-'));
...
test('all five renderers inherit one finite running-to-settled ambient contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /animation: archipam-edge-flow [^;]+ 1;/, mode);
    assert.match(html, /animation: archipam-node-pulse [^;]+ 1;/, mode);
    assert.doesNotMatch(html, /animation: archipam-edge-flow [^;]+ infinite/, mode);
    assert.doesNotMatch(html, /animation: archipam-node-pulse [^;]+ infinite/, mode);
  }
});
...
test('settled flow restores authored security and async dash semantics', () => {
  ...
  assert.match(template, /@keyframes archipam-edge-flow\s*\{[\s\S]*?stroke-dasharray:\s*10 8/);
