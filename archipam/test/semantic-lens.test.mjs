...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-semantic-lens-'));
...
test('all typed renderers inherit one viewer-only Semantic Lens', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /ArchiPam\.semanticLens = \(function \(\)/, mode);
...
test('Semantic Lens is shareable and yields cleanly to stronger reader intent', () => {
  ...
  assert.match(html, /ArchiPam\.semanticLens\.clear\(\{ updateUrl: false/);
  assert.match(html, /ArchiPam\.focus\.clear\(\{ updateUrl: false, preserveView: true \}\)/);
  assert.match(html, /ArchiPam\.routeProbe\.clear\(\{ updateUrl: false, restoreFocus: false \}\)/);
  assert.match(html, /ArchiPam\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /if \(action === 'lens'\) return ArchiPam\.semanticLens\.open\(\)/);
  assert.match(html, /e\.key === 'l' \|\| e\.key === 'L'/);
  assert.match(html, /e\.key === 'Escape' && ArchiPam\.semanticLens\.isOpen\(\)/);
  assert.match(html, /e\.key === 'Escape' && ArchiPam\.semanticLens\.active\(\)/);
});
