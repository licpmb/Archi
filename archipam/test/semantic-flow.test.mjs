...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-semantic-flow-'));
...
test('Semantic Flow has preset identities and motion-safe density boundaries', () => {
  ...
  assert.match(html, /@keyframes archipam-semantic-lens-flow/);
  assert.match(html, /animation: archipam-semantic-lens-flow 1\.35s linear 1 both/);
