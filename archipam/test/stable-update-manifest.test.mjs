...
test('stable release gate binds manifest tag, tree, and final archive digest', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-stable-manifest-'));
  try {
    writeJson(path.join(fixture, 'archipam', 'package.json'), { version: '3.0.0' });
    fs.writeFileSync(path.join(fixture, 'archipam', 'SKILL.md'), 'stable fixture\n');
    assert.equal(git(fixture, ['init']).status, 0);
    assert.equal(git(fixture, ['add', 'archipam']).status, 0);
    assert.equal(git(fixture, [
      '-c', 'user.name=ArchiPam Test',
      '-c', 'user.email=archipam@example.invalid',
    ...
    const tree = git(fixture, ['rev-parse', 'HEAD:archipam']);
    ...
    const manifestPath = path.join(fixture, 'docs', 'skill-updates', 'archipam', 'stable.json');
    const manifest = {
      schemaVersion: 1,
      skillId: 'archipam',
      channel: 'stable',
      ...
      releaseNotes: 'https://github.com/licpmb/Archi/releases/tag/v3.0.0',
      severity: 'normal',
    };
    ...
    writeJson(path.join(fixture, 'archipam', 'package.json'), { version: '4.0.0-dev.0' });
    ...
    assert.match(treeMismatch.stderr, /treeSha .* does not match HEAD:archipam/);
