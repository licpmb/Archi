...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-finder-'));
...
test('all typed renderers ship the same geometry-neutral node finder', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /ArchiPam\.finder = \(function \(\)/, mode);
    assert.match(html, /svg\.querySelectorAll\('\[data-node-id\]'\)/, mode);
    assert.doesNotMatch(svg(html), /node-finder|ArchiPam\.finder|Find a node/, mode);
  }
});
...
test('finder searches semantic ids and labels, then delegates to focus and reveal', () => {
  ...
  assert.match(html, /ArchiPam\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /ArchiPam\.view\.reset\(\{ automatic: true \}\)/);
  assert.match(html, /ArchiPam\.focus\.set\(id, \{ toggle: false \}\)/);
  assert.match(html, /ArchiPam\.view\.reveal\(\[id\], \{ includeNeighbors: true, reason: 'finder' \}\)/);
...
test('finder becomes a contextual Route Probe endpoint picker without changing semantic focus', () => {
  ...
  assert.match(html, /ArchiPam\.routeProbe\.finderContext\(\)/);
  ...
  assert.match(html, /ArchiPam\.routeProbe\.choose\(id\)/);
