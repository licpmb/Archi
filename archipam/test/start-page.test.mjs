...
function executeStartPage(html) {
  ...
  const context = {
    window,
    ArchiPamSiteLanguage: window.ArchiPamSiteLanguage,
    document,
...
test('start page: checked-in HTML is reproducible from canonical scenario recipes', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-start-page-'));
...
test('start page: offers five bounded bilingual starts without ingesting source content', () => {
  ...
  assert.match(html, /data-en="ArchiPam maps it\."/);
  assert.match(html, /data-zh="直接说，"/);
  assert.match(html, /data-zh="ArchiPam 就能画。"/);
  ...
  assert.match(scriptMatch[1], /archipam:start-funnel/);
  assert.match(scriptMatch[1], /archipam\.start\.events\.v1/);
...
test('start page: input mode drives rendered prompt, copy, keyboard, and URL without changing event schema', async () => {
  ...
  const [viewEvent, promptEvent, starterEvent] = page.window.ArchiPamStartMetrics.snapshot();
...
test('generated artifacts omit the promotional footer and shortcut manual', () => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-start-artifacts-'));
  try {
    for (const [type, input] of Object.entries(examples)) {
      ...
      assert.doesNotMatch(html, /Built with ArchiPam/, `${type}: product signature`);
...
test('viewer gives wide screens a larger canvas without forcing a subtitle row', () => {
  ...
  assert.match(template, /max-width: var\(--archipam-reader-width, 1440px\)/);
  assert.match(template, /ArchiPam\.readerLayout = \(function \(\)/);

  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-title-hierarchy-'));
  try {
