...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-diagram-guide-'));
...
test('all typed renderers inherit one viewer-only Diagram Guide', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /ArchiPam\.guide = \(function \(\)/, mode);
    assert.match(html, /Diagram Guide — a factual command deck over existing interactions/, mode);
    assert.doesNotMatch(canonicalSvg(html), /diagram-guide|ArchiPam\.guide|Explore this system/, mode);
  }
});
...
test('Diagram Guide reports compiled semantic facts and honest story availability', () => {
  ...
  assert.match(html, /return ArchiPam\.guidedViews && Number\(ArchiPam\.guidedViews\.count\) \|\| 0/);
...
test('Diagram Guide delegates its task rows to existing production interactions', () => {
  ...
  assert.match(html, /if \(action === 'find'\) return ArchiPam\.finder\.open\(\)/);
  assert.match(html, /if \(action === 'route'\) return ArchiPam\.routeProbe\.begin\(\{ focusNode: true \}\)/);
  assert.match(html, /if \(action === 'map'\) return ArchiPam\.radar\.open\(\)/);
  assert.match(html, /if \(action === 'story'\) return ArchiPam\.guidedViews\.play\(\)/);
  assert.match(html, /if \(action === 'present'\) return ArchiPam\.presentation\.enter\(\)/);
  assert.match(html, /if \(action === 'export'\) return ArchiPam\.exportMenu\.open\(\)/);
  assert.match(html, /if \(action === 'theme'\) return ArchiPam\.theme\.toggle\(\)/);
  assert.match(html, /if \(action === 'reset'\) return ArchiPam\.view\.reset\(\)/);
  assert.match(html, /ArchiPam\.guidedViews\.pause\(\)/);
  assert.match(html, /ArchiPam\.finder\.close\(\{ restoreFocus: false \}\)/);
  assert.match(html, /ArchiPam\.radar\.close\(\{ restoreFocus: false \}\)/);
...
test('Diagram Guide is keyboard-first, mobile-contained, motion-safe, and embed-clean', () => {
  ...
  assert.match(html, /ArchiPam\.guide\.toggle\(\)/);
