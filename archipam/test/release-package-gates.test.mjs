...
test('release prevents manifest preannouncement and smokes the exact archive before upload', () => {
  ...
  assert.match(tagGate, /require\('\.\/archipam\/package\.json'\)\.version/);
  ...
  assert.match(build, /run: scripts\/build-zip\.sh \/tmp\/archipam-built\.zip/);
  assert.match(smoke, /unzip -q \/tmp\/archipam-built\.zip -d "\$package_root"/);
  assert.match(smoke, /node scripts\/package-smoke\.mjs "\$package_root\/archipam"/);
  assert.doesNotMatch(smoke, /\bnpm\s+(?:ci|install)\b/);
  assert.match(freshness, /cmp -s \/tmp\/archipam-built\.zip archipam\.zip/);
  assert.match(upload, /files: archipam\.zip/);
  assert.match(followUp, /docs\/skill-updates\/archipam\/stable\.json/);
});

test('an exact tag fetch restores an annotated object after a SHA-only checkout', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-release-tag-fetch-'));
  ...
  try {
    ...
    assert.equal(runGit(source, ['config', 'user.name', 'ArchiPam Test']).status, 0);
    assert.equal(runGit(source, ['config', 'user.email', 'archipam@example.invalid']).status, 0);
...
test('CI binds a public notifier manifest to the Release asset, tagged archive, and tag tree build', () => {
  ...
  assert.match(job, /select\(\.name == "archipam\.zip"\)/);
  ...
  assert.match(job, /git show "v\$\{manifest_version\}:archipam\.zip" > "\$tagged_archive"/);
...
test('GitHub Pages deploys docs only after every repository gate succeeds', () => {
  ...
  assert.match(job, /actions\/configure-pages@983d7736d9b0ae728b81ab479565c72886d7745b/);
  assert.match(job, /actions\/upload-pages-artifact@7b1f4a764d45c48632c6b24a0339c27f5614fb0b/);
  assert.match(job, /path: docs/);
  assert.match(job, /actions\/deploy-pages@d6db90164ac5ed86f2b6aed7e0febac5b3c0c03e/);
});
...
test('package smoke rejects every dependency or repository-only artifact', () => {
  ...
  for (const { relative, kind } of forbidden) {
    const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-package-gate-'));
    try {
      fs.mkdirSync(path.join(fixture, 'bin'), { recursive: true });
      fs.writeFileSync(path.join(fixture, 'bin', 'archipam.mjs'), '');
...
test('package smoke verifies the embedded notifier identity and local disable switch', () => {
  ...
  assert.match(source, /ARCHIPAM_UPDATE_CHECK_DISABLED: '1'/);
...
test('package smoke increments an arbitrary-precision SemVer patch without Number coercion', () => {
  const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-package-bigint-version-'));
  const skillRoot = path.join(scratch, 'archipam');
  try {
...
test('archive build refuses to silently omit required notifier files', () => {
  ...
  assert.match(stageSource, /archipam\/skill-release\.json/);
  assert.match(stageSource, /archipam\/scripts\/check-update\.mjs/);
  assert.match(stageSource, /archipam\/scripts\/update-contract\.mjs/);
...
canonicalZipTest('package smoke rejects every dependency metadata field in a built package', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-built-package-gate-'));
  try {
    const archive = path.join(fixture, 'archipam.zip');
    ...
    const builtPackage = path.join(extracted, 'archipam');
...
canonicalZipTest('built archives contain the embedded notifier runtime', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-notifier-package-gate-'));
  try {
    const archive = path.join(fixture, 'archipam.zip');
    const build = spawnSync(path.join(repoRoot, 'scripts', 'build-zip.sh'), [archive], {
      cwd: repoRoot,
    ...
    assert.ok(entries.has('archipam/skill-release.json'));
    assert.ok(entries.has('archipam/scripts/check-update.mjs'));
    assert.ok(entries.has('archipam/scripts/update-contract.mjs'));
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});
...
canonicalZipTest('archive build excludes untracked files and external symlinks from the live working tree', () => {
  ...
  const untracked = path.join(repoRoot, 'archipam', `${marker}.txt`);
  const externalRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-package-external-'));
  const externalTarget = path.join(externalRoot, 'secret.txt');
  const externalLink = path.join(repoRoot, 'archipam', `${marker}.link`);
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-package-negative-'));
  const archive = path.join(outputRoot, 'archipam.zip');
...
canonicalZipTest('archive build rejects an unmerged index and preserves an existing archive', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-package-unmerged-'));
  const scripts = path.join(fixture, 'scripts');
  const skill = path.join(fixture, 'archipam');
  ...
  try {
    ...
    fs.writeFileSync(path.join(skill, 'package.json'), '{"name":"archipam"}\n');
    ...
    const indexInfo = [
      `100644 ${base.stdout.trim()} 1\tarchipam/LICENSE`,
      `100644 ${ours.stdout.trim()} 2\tarchipam/LICENSE`,
      `100644 ${theirs.stdout.trim()} 3\tarchipam/LICENSE`,
      '',
    ].join('\n');
...
test('archive build rejects non-canonical Node versions before publishing output', {
  ...
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-package-node-version-'));
  try {
    const archive = path.join(outputRoot, 'archipam.zip');
    ...
    assert.notEqual(build.status, 0, `${build.stdout}\n${build.stderr}`);
    assert.match(build.stderr, /canonical archipam\.zip builds require Node 22/);
...
canonicalZipTest('archive build is byte-for-byte reproducible across caller time zones without system zip', () => {
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-package-reproducible-'));
  ...
  try {
    ...
    assert.ok(
      fs.readFileSync(utcArchive).equals(fs.readFileSync(path.join(repoRoot, 'archipam.zip'))),
...
test('CI tests the declared Node floor plus every maintained current lane', () => {
  ...
  assert.equal(packageJson.engines?.node, '^22.19.0 || >=24.0.0 <25');
  ...
  assert.match(packageSmokeJob, /node-version:\s*24/);
});
