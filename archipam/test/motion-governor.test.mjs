...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-motion-governor-'));
...
test('all renderers inherit one viewer-only Live/Still Motion Governor', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /ArchiPam\.motionGovernor = \(function \(\)/, mode);
...
test('reader pause is persistent, explicit, and reduced-motion aware', () => {
  ...
  assert.match(html, /var STORAGE_KEY = 'archipam-motion'/);
  ...
  assert.match(html, /ArchiPam\.guidedViews\.isPlaying\(\)[\s\S]*?ArchiPam\.guidedViews\.pause\(\)/);
...
test('strong semantic intent receives the single motion budget', () => {
  ...
  assert.match(html, /animation: archipam-route-probe-flow 1\.1s[\s\S]*?1 both/);
  assert.doesNotMatch(html, /archipam-route-probe-flow[^;]*infinite/);
});
