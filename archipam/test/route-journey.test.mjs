...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-route-journey-'));
...
test('motion, camera, layered Escape, mobile, print, and embed boundaries stay explicit', () => {
  assert.match(template, /ArchiPam\.motionGovernor\.capable === true[\s\S]*?!ArchiPam\.motionGovernor\.isPaused\(\)/);
  assert.match(template, /ArchiPam\.motionGovernor\.claim\('route'/);
  assert.match(template, /reason: 'route-journey',[\s\S]*?maxScale: 1\.65,[\s\S]*?padding: 64,[\s\S]*?duration: 360/);
  assert.match(template, /ArchiPam\.routeProbe\.pauseJourney\(\{ preserveElapsed: true, reason: reason \|\| 'manual' \}\)/);
  ...
  assert.match(template, /ArchiPam\.routeProbe\.escape\(\{ restoreFocus: true \}\)/);
