...
const cursorCommand = 'npx -y skills add licpmb/Archi --skill archipam --agent cursor --global --copy --yes';
...
test('Cursor onboarding stays explicit, bilingual, and backed by the same Skill', () => {
  ...
  for (const surface of [english, chinese, start, landing]) {
    ...
    assert.doesNotMatch(surface, /~\/\.cursor\/skills\/archipam/);
...
test('the zero-dependency archive works from the canonical Cursor-visible agent path', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-cursor-package-'));
  const agentSkills = path.join(tmp, '.agents', 'skills');
  try {
    fs.mkdirSync(agentSkills, { recursive: true });
    execFileSync('unzip', ['-q', path.join(repoRoot, 'archipam.zip'), '-d', agentSkills]);
    const installed = path.join(agentSkills, 'archipam');
    const cli = path.join(installed, 'bin', 'archipam.mjs');
    const doctor = execFileSync(process.execPath, [cli, 'doctor'], { encoding: 'utf8' });
    assert.match(doctor, /ArchiPam is ready\./);
