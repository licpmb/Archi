...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-cli-'));
...
fs.writeFileSync(process.env.ARCHIPAM_TEST_OPEN_LOG, JSON.stringify({
...
test('cli: help lists commands and diagram types', () => {
  ...
  assert.match(result.stdout, /archipam render <type>/);
  assert.match(result.stdout, /archipam compare architecture <base\.json> <head\.json>/);
  assert.match(result.stdout, /archipam deliver <type>/);
  assert.match(result.stdout, /archipam preview <type>/);
  assert.match(result.stdout, /archipam visual-check <output\.html>/);
  assert.match(result.stdout, /--open/);
  assert.match(result.stdout, /--repo-root path \(architecture only\)/);
  assert.match(result.stdout, /archipam guide \[scenario or question\]/);
  assert.match(result.stdout, /archipam doctor/);
  assert.match(result.stdout, /archipam demo \[output-directory\]/);
...
test('cli: doctor reports a complete installation is ready', () => {
  ...
  assert.match(result.stdout, /ArchiPam is ready\./);
});
...
test('cli: doctor identifies an incomplete installation', () => {
  ...
  fs.copyFileSync(cli, path.join(incompleteBin, 'archipam.mjs'));

  const result = spawnSync(process.execPath, [path.join(incompleteBin, 'archipam.mjs'), 'doctor'], {
    cwd: incompleteRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /ArchiPam is not ready: \d+ required files? missing\./);
});
...
test('cli: doctor rejects a corrupt standalone validator', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(corruptRoot, 'bin/archipam.mjs'), 'doctor'], {
    cwd: corruptRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /ArchiPam is not ready: 1 runtime check failed\./);
});
...
test('cli: examples renders from an installed skill', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(installedRoot, 'bin/archipam.mjs'), 'examples'], {
    cwd: installedRoot,
    encoding: 'utf8',
  });
...
test('cli: guide lists all scenario recipes by diagram type', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /ArchiPam scenario recipes \(11\)/);
...
test('cli: guide works from an installed skill without node_modules', () => {
  ...
  copyInstalledSkill(installedRoot);
  const installedCli = path.join(installedRoot, 'bin/archipam.mjs');
...
test('cli: demo creates a ready-to-open diagram in a chosen directory', () => {
  ...
  assert.match(result.stdout, /archipam render architecture/);
});
...
test('cli: demo defaults to the current directory', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.equal(fs.existsSync(path.join(workingDirectory, 'archipam-demo.html')), true);
});
...
test('cli: visual-check returns a skipped receipt with exit 2 when Chrome is unavailable', () => {
  ...
  const result = run(['visual-check', out, '--json'], {
    env: { ...process.env, ARCHIPAM_CHROME: missingChrome },
  });
...
test('cli: preview runs from an installed skill without node_modules and exits cleanly', { timeout: 30000 }, async () => {
  ...
  assert.deepEqual(fs.readdirSync(path.dirname(output)).filter((name) => name.startsWith('.archipam-preview-')), []);
});
...
test('cli: deliver preserves the previous artifact when the final check fails', () => {
  ...
  assert.deepEqual(
    fs.readdirSync(path.dirname(out)).filter((name) => name.includes('.archipam-delivery-')),
    [],
  );
});
...
test('cli: inspect emits architecture layout json', () => {
  const input = path.resolve(skillRoot, '../examples/archipam-repo-grid.architecture.json');
