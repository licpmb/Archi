...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-guided-views-'));
...
for (const [mode, config] of Object.entries(CASES)) {
  test(`${mode}: guided views preserve base SVG geometry`, () => {
    ...
    assert.match(guided.html, /ArchiPam\.guidedViews = \(function \(\)/);
    ...
    assert.match(guided.html, /--archipam-scroll-x/);
