...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-delta-'));
...
test('compare CLI writes a deterministic three-state artifact and complete sidecar receipt', () => {
  ...
  assert.match(firstHtml, /window\.ArchiPam\.deltaExport = \{ canonicalSvg: canonicalDeltaSvg, shareCard/);
  ...
  assert.match(firstHtml, /document\.querySelectorAll\('#archipam-compare-receipt'\)\.length !== 1/);
...
test('artifact validation fails closed on missing, duplicate, or self-blessed review identity', () => {
  ...
  const receiptNode = html.match(/<script id="archipam-compare-receipt"[\s\S]*?<\/script>/)?.[0];
