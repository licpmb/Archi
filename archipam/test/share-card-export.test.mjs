...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-share-card-'));
...
test('Share Card uses contain-only canonical geometry with fixed safe areas', () => {
  ...
  assert.match(html, /ARCHIPAM ·/);
  assert.doesNotMatch(svgBlock(html), /share-card|Share Card|ARCHIPAM ·/);
});
