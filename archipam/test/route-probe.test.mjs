...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-route-probe-'));
...
test('all typed renderers inherit one viewer-only Route Probe', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /ArchiPam\.routeProbe = \(function \(\)/, mode);
...
test('Route Probe turns a two-node question into a readable route receipt and stable link', () => {
  ...
  assert.match(html, /ArchiPam\.view\.reveal\(result\.nodes, \{ includeNeighbors: false, reason: 'route' \}\)/);
...
test('Route Probe hands large-diagram endpoint selection to a reachability-aware Finder', () => {
  ...
  assert.match(html, /ArchiPam\.finder\.open\(\{ context: context \}\)/);
...
test('Route Probe keeps pointer, keyboard, motion, embed, and export boundaries explicit', () => {
  ...
  assert.match(html, /e\.key === 'Escape' && ArchiPam\.routeProbe\.active\(\)/);
  ...
  assert.match(html, /@keyframes archipam-route-probe-flow/);
