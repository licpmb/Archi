...
const assetPath = path.join(repoRoot, 'docs', 'assets', 'archipam-live-proof.gif');
const receiptPath = path.join(repoRoot, 'docs', 'assets', 'archipam-live-proof.json');
...
test('README motion proof is compact, looping, and backed by current gallery artifacts', () => {
  ...
  assert.equal(receipt.output, 'docs/assets/archipam-live-proof.gif');
...
test('all README languages keep the product hero and retain the verified animated proof', () => {
  for (const filename of ['README.md', 'README_EN.md', 'README_ZH.md']) {
    ...
    const heroIndex = readme.indexOf('docs/assets/archipam-readme-hero.png');
    const titleIndex = readme.indexOf('# ArchiPam');
    const proofIndex = readme.indexOf('docs/assets/archipam-live-proof.gif');
    const demosIndex = Math.max(readme.indexOf('## See ArchiPam in action'), readme.indexOf('## 看看 ArchiPam 能做什么'));
    ...
    assert.match(readme, /docs\/assets\/archipam-live-proof\.gif/);
    assert.match(readme, /https:\/\/tt-a1i\.github\.io\/archipam\/gallery\.html/);
  }
  assert.equal(
...
test('README stays scannable without deleting the visual proof set', () => {
  const commonAssets = [
    'archipam-readme-hero.png',
    'archipam-live-proof.gif',
    'archipam-demo-story.png',
    'archipam-demo-route.png',
    'archipam-demo-lens.png',
    'mco-runtime-share-card.png',
    'archipam-dark.png',
    'archipam-light.png',
    'archipam-menu.png',
    'archipam-workflow.png',
    'archipam-sequence.png',
    'archipam-dataflow.png',
    'archipam-lifecycle.png',
  ];
...
test('all README languages end with the self-hosted star history chart', () => {
  const lightChart = 'https://raw.githubusercontent.com/licpmb/Archi/star-history/assets/star-history-light.svg';
  const darkChart = 'https://raw.githubusercontent.com/licpmb/Archi/star-history/assets/star-history-dark.svg';
...
test('Star History publishing advances the data branch without a force push', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-star-history-'));
