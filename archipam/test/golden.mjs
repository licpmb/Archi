// Golden-file harness for the archipam renderers. No test framework needed:
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-test-'));
...
for (const [mode, input, golden] of GOLDEN) {
  ...
  try {
    ...
    check(`${mode}: packaged ${golden}`, normalizeNewlines(fresh) === normalizeNewlines(packaged),
      `fresh render differs from archipam/examples/${golden}; re-render the packaged examples and rebuild archipam.zip`);
  } catch (err) {
...
check('template generator meta matches package.json version',
  template.includes(`<meta name="generator" content="archipam ${pkg.version}">`),
