...
test('shared release constants identify the only trusted updater source', () => {
  assert.equal(SKILL_ID, 'archipam');
  assert.equal(EXPECTED_REPOSITORY, 'https://github.com/licpmb/Archi');
  assert.equal(
    DEFAULT_MANIFEST_URL,
...
test('stable manifests enforce one exact schema and canonical release identity', () => {
  ...
  const invalid = [
    ...
    stableManifest({ releaseNotes: 'https://github.com:443/licpmb/Archi/releases/tag/v2.16.0' }),
    stableManifest({ releaseNotes: 'https://github.com/licpmb/Archi/releases/tag/v2.16.0?' }),
    stableManifest({ releaseNotes: 'https://GITHUB.COM/licpmb/Archi/releases/tag/v2.16.0' }),
  ];
