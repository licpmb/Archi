...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-relationship-pulse-'));
...
test('semantic tokens use five distinct inline SVG cues on one finite timing owner', () => {
  ...
  assert.match(html, /animation: archipam-relationship-token-life 1\.2s linear 1 both/);
  assert.match(html, /semantic-flow-token-halo/);
  assert.match(html, /ArchiPam\.flowTokens = \{/);
...
test('pulse is finite, event-owned, preset-aware, touch-safe, and motion-safe', () => {
  ...
  assert.match(html, /animation: archipam-relationship-pulse 1\.2s linear 1 both/);
  assert.match(html, /@keyframes archipam-relationship-token-life/);
  ...
  assert.match(html, /ArchiPam\.motionGovernor && ArchiPam\.motionGovernor\.isPaused\(\)/);
