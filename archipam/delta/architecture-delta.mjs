...
export function compareArchitecture(base, head, evidence = {}) {
  ...
  if (!shared.length) {
    fail('delta/no-shared-component-id', 'The snapshots share no component id, so ArchiPam cannot prove that they describe the same system.', {
...
<script id="archipam-compare-receipt" type="application/json">${safeJson(receipt)}</script>
<script>(()=>{
  ...
  const receiptNode = document.querySelector('#archipam-compare-receipt');
  ...
  function validateReview() {
    try {
      if (document.querySelectorAll('#archipam-compare-receipt').length !== 1 || !receiptNode) return false;
  ...
  function recordExport(format, blob, dimensions) {
    document.documentElement.dataset.archipamDeltaExport = JSON.stringify({
      ok: true,
      format,
      bytes: blob.size,
      mime: blob.type,
  ...
  window.ArchiPam = window.ArchiPam || {};
  window.ArchiPam.deltaExport = { canonicalSvg: canonicalDeltaSvg, shareCard, exportSvg: exportCanonicalSvg, downloadShareCard };
  window.ArchiPam.exportMenu = {
    shareCard,
    run(format) {
...
export function validateArchitectureDeltaHtml(html, receipt) {
  ...
  if ((html.match(/id="archipam-compare-receipt"/g) || []).length !== 1) failures.push('expected exactly one embedded compare receipt');
  ...
  if (!html.includes('id="export-svg"') || !html.includes('id="share-card"')
    || !html.includes('window.ArchiPam.deltaExport = { canonicalSvg: canonicalDeltaSvg, shareCard')) {
