...
export function loadDiagram({ rendererDir, diagramType, defaultExample, argv = process.argv }) {
  ...
  const sourceEvidence = verifyRepositoryEvidence(diagramType, diagram, process.env.ARCHIPAM_REPO_ROOT);
...
export function svgRootAttrs(meta) {
  ...
  const engineeringProfile = meta.engineering_profile
    ...
    : '';
  const requestedProfile = process.env.ARCHIPAM_QUALITY_PROFILE || meta.quality_profile;
  ...
  return `role="img" lang="${esc(resolveLocale(meta.locale))}" aria-labelledby="archipam-diagram-title archipam-diagram-description"${animation}${preset}${engineeringProfile} data-quality-profile="${esc(qualityProfile)}"${advisory}`;
}

// Keep the accessible name inside the SVG so it survives standalone SVG
// export and embedding. The fixed IDs are deterministic because an ArchiPam
...
export function svgAccessibleText(meta, kind) {
  ...
  return `        <title id="archipam-diagram-title">${esc(meta.title)}</title>\n        <desc id="archipam-diagram-description">${esc(description)}</desc>`;
}
...
// canonical SVG is embedded inline outside the full ArchiPam viewer.
