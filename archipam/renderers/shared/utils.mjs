...
const SVG_SLOT_RE = /      <!-- ARCHIPAM:SVG_SLOT_START -->[\s\S]*?      <!-- ARCHIPAM:SVG_SLOT_END -->/;
const CARDS_SLOT_RE = /    <!-- ARCHIPAM:CARDS_SLOT_START -->[\s\S]*?    <!-- ARCHIPAM:CARDS_SLOT_END -->/;
const SUBTITLE_SLOT_RE = /^([ \t]*)<p class="subtitle">\[Subtitle description\]<\/p>[ \t]*(\r?\n)?/m;
const GUIDED_VIEWS_PLACEHOLDER = '<!-- ARCHIPAM:GUIDED_VIEWS_DATA -->';
const SOURCE_EVIDENCE_PLACEHOLDER = '    <!-- ARCHIPAM:SOURCE_EVIDENCE_DATA -->';
const I18N_PLACEHOLDER = '    <!-- ARCHIPAM:I18N_DATA -->';
...
export function applyTemplate(template, {
  ...
  if (!SVG_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing ARCHIPAM:SVG_SLOT sentinel');
  }
  if (!CARDS_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing ARCHIPAM:CARDS_SLOT sentinel');
  }
  ...
  const renderedSubtitle = typeof subtitle === 'string' && subtitle.trim()
    ...
    : '';
  const i18nData = `    <script id="archipam-i18n-data" type="application/json">${i18nJson}</script>`;
  ...
  return templateWithI18n
    ...
    .replace(GUIDED_VIEWS_PLACEHOLDER, () => `<script id="archipam-guided-views-data" type="application/json">${guidedViewsJson}</script>`)
    .replace(SOURCE_EVIDENCE_PLACEHOLDER, () => sourceEvidence
      ? `    <script id="archipam-source-evidence-data" type="application/json">${sourceEvidenceJson}</script>`
      : '');
}
