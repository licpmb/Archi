...
const DIAGNOSTIC_MODE = process.env.ARCHIPAM_DIAGNOSTIC_FORMAT === 'json';
const recorded = [];
const recordedMessages = new Set();
const boundaryKey = Symbol.for('archipam.renderer-diagnostic-boundary');
...
function normalizedDiagnostic(diagnostic) {
  const message = String(diagnostic?.message || 'ArchiPam could not classify this failure.').trim();
  return {
...
export function throwDiagnosticError(message, diagnostics) {
  ...
  error.archipamDiagnostics = (diagnostics || []).map(normalizedDiagnostic);
  throw error;
}
...
function rendererFailure(error) {
  const attached = Array.isArray(error?.archipamDiagnostics)
    ? error.archipamDiagnostics.map(normalizedDiagnostic)
    : [];
