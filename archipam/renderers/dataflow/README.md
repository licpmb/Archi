...
Render `diagram_type: "dataflow"` JSON files into the standard ArchiPam HTML
template.

```bash
node archipam/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `archipam/schemas/dataflow.schema.json`
...
`archipam/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
archipam/schemas/dataflow.schema.json
```
...
visual-only because ArchiPam has no compiled edge-kind facts in this slice. A
