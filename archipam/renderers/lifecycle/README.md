...
Render `diagram_type: "lifecycle"` JSON files into the standard ArchiPam HTML
template.

```bash
node archipam/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

The renderer validates input against `archipam/schemas/lifecycle.schema.json`
...
`archipam/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
archipam/schemas/lifecycle.schema.json
```
