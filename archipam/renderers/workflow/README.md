...
Render `diagram_type: "workflow"` JSON files into the standard ArchiPam HTML
template.

```bash
node archipam/renderers/workflow/render-workflow.mjs input.workflow.json output.html
```

The renderer validates input against `archipam/schemas/workflow.schema.json`
...
```bash
node archipam/scripts/check-render-output.mjs output.html
```
...
`archipam/examples/agent-tool-call.workflow.json`; its `schema_version` selects
the applicable contract.

The schema lives at:

```text
archipam/schemas/workflow.schema.json
```
...
```bash
node archipam/bin/archipam.mjs migrate workflow old.json new.json --to-schema 2 --json
```
...
```bash
node archipam/bin/archipam.mjs validate workflow input.workflow.json --layout-json
```
