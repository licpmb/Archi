...
function compileWorkflowInternal({
  ...
  try {
    ...
    inputDiagnostics = Array.isArray(error?.archipamDiagnostics)
      ? error.archipamDiagnostics.map((diagnostic) => ({
          ...diagnostic,
          supportedFixes: [],
        }))
      : [{
...
function readableAutomaticRoute(edge, from, to, primarySides, primaryPorts) {
  ...
  if (authoredSideFields.length) {
    ...
    const attemptedEvidence = firstFailure?.archipamDiagnostics?.[0]?.evidence || {};
...
${renderLegend()}
  ...
  try {
    ...
    if (!Array.isArray(error?.archipamDiagnostics)) throw error;
    const diagnostics = error.archipamDiagnostics.map((diagnostic) => ({ ...diagnostic }));
