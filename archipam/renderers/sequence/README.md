...
Render `diagram_type: "sequence"` JSON files into the standard ArchiPam HTML
template.

```bash
node archipam/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

The renderer validates input against `archipam/schemas/sequence.schema.json`
...
`archipam/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
archipam/schemas/sequence.schema.json
```
