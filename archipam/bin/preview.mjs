...
const cliPath = path.join(here, 'archipam.mjs');
...
<head>
  ...
  <title>ArchiPam Live Preview</title>
  <style>
...
<body data-state="checking" data-has-artifact="false">
  <header>
    <span class="brand">ArchiPam Preview</span>
  ...
  <main>
    ...
    <iframe id="artifact" title="Verified ArchiPam diagram"></iframe>
  </main>
  <script>
    (function () {
      'use strict';
...
export async function startPreview(options) {
  ...
  const stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.archipam-preview-'));
    ...
    state.failure = {
      ...
      message: redactDiagnostic(
        ...
        [
          ...
          [path.resolve(here, '..'), '<archipam-skill>'],
