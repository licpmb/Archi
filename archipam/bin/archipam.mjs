...
function usage() {
  return `Usage:
  archipam render <type> <input.json> [output.html] [--quality standard|showcase] [--repo-root path (architecture only)]
  archipam compare architecture <base.json> <head.json> [output.html] [--receipt path] [--json] [--quality standard|showcase] [--repo-root path]
  archipam deliver <type> <input.json> [output.html] [--json] [--open] [--quality standard|showcase] [--repo-root path (architecture only)]
  archipam preview <type> <input.json> [output.html] [--no-open] [--quality standard|showcase] [--repo-root path (architecture only)]
  archipam validate <type> <input.json> [--json] [--layout-json] [--quality standard|showcase] [--repo-root path (architecture only)]
  archipam migrate workflow <old.json> <new.json> --to-schema 2 [--json]
  archipam inspect <type> <input.json>
  archipam check <output.html>
  archipam visual-check <output.html> [--json]
  archipam guide [scenario or question] [--json] [--lang en|zh]
  archipam brands [name, alias, domain, or category] [--json]
  archipam brands capture <url> [--json]
  archipam examples
  archipam doctor
  archipam demo [output-directory]
...
function rendererEnv(quality, repoRoot, diagnosticJson = false) {
  return {
    ...(quality ? { ARCHIPAM_QUALITY_PROFILE: quality } : {}),
    ...(repoRoot ? { ARCHIPAM_REPO_ROOT: repoRoot } : {}),
    ...(diagnosticJson ? { ARCHIPAM_DIAGNOSTIC_FORMAT: 'json' } : {}),
  };
}
...
async function commandCompare(args) {
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'prepare', error: 'Architecture compare runtime is unavailable.', code: 'delta/runtime-missing', details: { reason: error.message, supportedFixes: ['install the complete ArchiPam skill package'] } });
    return;
  }
  const {
  ...
  try {
    ...
    const outputDiagnostic = error.archipamDiagnostics?.[0];
  ...
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.archipam-compare-'));
  } catch (error) {
...
function sourceEvidenceFromArtifact(artifact) {
  ...
  const match = html.match(/<script id="archipam-source-evidence-data" type="application\/json">([\s\S]*?)<\/script>/);
...
async function commandDeliver(args) {
  ...
  try {
    ...
    reportDeliveryFailure({
      ...
      error: error.message,
      diagnostics: error.archipamDiagnostics || [diagnostic({
  ...
  let stagingDirectory;
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.archipam-delivery-'));
  } catch (error) {
...
async function commandDoctor() {
  ...
  const problems = [];
  if (nodeFailed) problems.push('Node.js 22.19+ or Node.js 24.x is required');
  ...
  console.error(`\nArchiPam is not ready: ${problems.join('; ')}.`);
  process.exitCode = 1;
}
...
async function commandBrands(args) {
  ...
  if (positional[0] === 'capture') {
    if (positional.length !== 2) fail('Usage: archipam brands capture <url> [--json]');
  ...
  if (json) {
    console.log(JSON.stringify({
      ...
      count: marks.length,
      marks,
      fallback: 'Run "archipam brands capture <url> --json", then use the returned digest-pinned brand value.',
    }, null, 2));
    return;
  }
  if (!marks.length) {
    console.log(`No built-in brand matched "${query}". Run "archipam brands capture <url> --json", then use the returned digest-pinned brand value.`);
    return;
  }
...
function commandDemo(args) {
  ...
  const output = path.join(outputDirectory, 'archipam-demo.html');
  ...
  console.log('  archipam render architecture <input.json> <output.html>');
}

function migrationPathDiagnostics(error, sourcePath, destinationPath) {
  if (Array.isArray(error?.archipamDiagnostics) && error.archipamDiagnostics.length) {
    return error.archipamDiagnostics.map((entry) => ({
      ...entry,
...
async function commandMigrate(args) {
  ...
  if (
    ...
    fail('Usage: archipam migrate workflow <old.json> <new.json> --to-schema 2 [--json]');
  }
  ...
  try {
    ...
    migration = {
      ...
      migrationDiagnostics: [diagnostic({
        ...
        evidence: { reason: error.message },
        supportedFixes: ['report the source workflow and this diagnostic to the ArchiPam maintainers'],
      })],
    };
  }
  ...
  try {
    ...
    stagingDirectory = fs.mkdtempSync(path.join(destinationDirectory, '.archipam-migration-'));
  } catch (error) {
    reportMigrationFailure({
      ...migration,
      migrationDiagnostics: [...migration.migrationDiagnostics, diagnostic({
  ...
  try {
    ...
    const migrationDiagnostics = Array.isArray(error?.archipamDiagnostics)
...
function commandValidate(args) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'archipam-validate-'));
